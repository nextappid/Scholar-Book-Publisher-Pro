...
class SBPP_Metadata_Generator {
    ...
    private function generate_book_metadata() {
        ...
        if ($pdf_available && $access_category === 'open') {
            $pdf_source = get_post_meta($post->ID ?? 0, '_sbpp_pdf_source', true);
            if ($pdf_source === 'wordpress') {
                $pdf_id = get_post_meta($post->ID ?? 0, '_sbpp_pdf_wordpress_id', true);
                if ($pdf_id) $pdf_url = wp_get_attachment_url($pdf_id);
            } elseif ($pdf_source === 'gdrive') {
                $pdf_gdrive_id = get_post_meta($post->ID ?? 0, '_sbpp_pdf_gdrive_id', true);
    ...
    private function generate_chapter_metadata() {
        ...
        if (!empty($isbn)) echo '<meta name="citation_isbn" content="' . esc_attr($isbn) . '">' . "\n";
        $permalink = get_permalink();
        if (!empty($permalink)) echo '<meta name="citation_fulltext_html_url" content="' . esc_url($permalink) . '">' . "\n";
