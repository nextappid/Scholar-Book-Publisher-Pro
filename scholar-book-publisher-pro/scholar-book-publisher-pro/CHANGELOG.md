# Changelog - Scholar Book Publisher Pro
...
- `_sbp_book_subtitle` - Book subtitle
- `_sbp_authors` - Array of authors
- `_sbp_editors` - Array of editors
- `_sbp_book_publisher` - Publisher name
- `_sbp_publisher_city` - **Publisher city (NEW)**
- `_sbp_publication_date` - Publication date
- `_sbp_isbn` - ISBN number
- `_sbp_doi` - DOI identifier
- `_sbp_pdf_available` - Boolean (PDF available or not)
- `_sbp_pdf_source` - PDF source type (wordpress/gdrive)
- `_sbp_pdf_wordpress_id` - WordPress media ID
- `_sbp_pdf_gdrive_url` - Google Drive share URL
- `_sbp_pdf_gdrive_id` - Extracted Google Drive File ID
- `_sbp_pdf_file_size` - PDF file size in MB
- `_sbp_pdf_size_warning` - Warning message for oversized files
