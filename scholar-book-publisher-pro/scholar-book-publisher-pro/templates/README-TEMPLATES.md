...
```php
// In plugin main file
function sbp_enqueue_custom_styles() {
...
add_action('wp_enqueue_scripts', 'sbp_enqueue_custom_styles');
```
...
3. Dequeue theme styles on book pages:
   ```php
   function sbp_dequeue_theme_styles() {
       if (is_singular('scholar_book')) {
           wp_dequeue_style('theme-style-handle');
       }
   }
   add_action('wp_enqueue_scripts', 'sbp_dequeue_theme_styles', 100);
   ```
