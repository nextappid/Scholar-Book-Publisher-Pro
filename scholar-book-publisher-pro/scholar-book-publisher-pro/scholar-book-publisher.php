...
final class Scholar_Book_Publisher {
    ...
    public function includes() {
        ...
        require_once SBP_PLUGIN_DIR . 'includes/class-sbp-metadata.php';
        require_once SBP_PLUGIN_DIR . 'includes/class-sbp-crawler-optimizer.php';
        require_once SBP_PLUGIN_DIR . 'includes/class-sbp-admin-notices.php';
        require_once SBP_PLUGIN_DIR . 'includes/class-sbp-usage-metrics.php';
        require_once SBP_PLUGIN_DIR . 'includes/class-sbp-seo-migration.php';
        require_once SBP_PLUGIN_DIR . 'includes/class-sbp-sitemap.php';
    }
    ...
    private function init_hooks() {
        ...
        add_action('wp_ajax_sbp_filter_books', array($this, 'ajax_filter_books'));
        add_action('wp_ajax_nopriv_sbp_filter_books', array($this, 'ajax_filter_books'));
        
        // AJAX handler for dismissing admin notice
        add_action('wp_ajax_sbp_dismiss_url_notice', array($this, 'dismiss_url_notice'));
        add_action('wp_ajax_sbp_dismiss_sitemap_notice', array($this, 'dismiss_sitemap_notice'));
    ...
    public function ajax_filter_books() {
        check_ajax_referer('sbp_filter_nonce', 'nonce');
        ...
        // Year filter via meta — use LIKE on _sbp_publication_date (format: YYYY-MM-DD or YYYY)
        if (!empty($year)) {
            $args['meta_query'][] = array(
                'key'     => '_sbp_publication_date',
                'value'   => $year,
                'compare' => 'LIKE',
            );
        }
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                $book_id       = get_the_ID();
                $cover_id      = get_post_meta($book_id, '_sbp_book_cover', true);
                $authors_raw   = get_post_meta($book_id, '_sbp_authors', true);
                $pub_date      = get_post_meta($book_id, '_sbp_publication_date', true);
                $year_val      = $pub_date ? date('Y', strtotime($pub_date)) : '';
                $access        = get_post_meta($book_id, '_sbp_access_category', true);
                ...
                // Metrics
                $metrics  = class_exists('SBP_Usage_Metrics') ? SBP_Usage_Metrics::get_metrics($book_id) : array('views' => 0, 'downloads' => 0);
    ...
    public function load_plugin_textdomain() {
        load_plugin_textdomain(
            ...
            false,
            dirname(SBP_PLUGIN_BASENAME) . '/languages/'
        );
    }
    ...
    public function enqueue_frontend_assets() {
        ...
        if (is_singular('scholar_book') || is_post_type_archive('scholar_book') || 
            ...
            wp_enqueue_style(
                'sbp-frontend',
                SBP_PLUGIN_URL . 'assets/css/frontend.css',
                array(),
                SBP_VERSION
            );
            ...
            wp_enqueue_script(
                'sbp-frontend',
                SBP_PLUGIN_URL . 'assets/js/frontend.js',
                array('jquery'),
                SBP_VERSION,
                true
            );
            ...
            wp_localize_script('sbp-frontend', 'sbp_ajax', array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce'    => wp_create_nonce('sbp_filter_nonce'),
            ));
        }
    }
    ...
    public function enqueue_admin_assets($hook) {
        ...
        if (($hook === 'post.php' || $hook === 'post-new.php') && 
            ...
            wp_enqueue_style(
                'sbp-admin',
                SBP_PLUGIN_URL . 'assets/css/admin.css',
                array(),
                SBP_VERSION
            );
            ...
            wp_enqueue_script(
                'sbp-admin',
                SBP_PLUGIN_URL . 'assets/js/admin.js',
                array('jquery'),
                SBP_VERSION,
                true
            );
    ...
    public function dismiss_url_notice() {
        check_ajax_referer('sbp_dismiss_notice', 'nonce');
        update_option('sbp_url_structure_notice_dismissed', true);
    ...
    public function dismiss_sitemap_notice() {
        check_ajax_referer('sbp_dismiss_sitemap', 'nonce');
        update_option('sbp_sitemap_notice_dismissed', true);
        wp_send_json_success();
    }
...
function sbp_activate_plugin() {
    ...
    require_once SBP_PLUGIN_DIR . 'includes/class-sbp-activator.php';
    SBP_Activator::activate();
}
