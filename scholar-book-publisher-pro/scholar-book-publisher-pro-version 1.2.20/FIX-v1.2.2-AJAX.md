...
- ✅ Added inline `sbpp_ajax` variables in template header
...
**Problem:**
AJAX filters only worked in WordPress admin because the `sbpp_ajax` variables (ajax_url and nonce) were being localized via `wp_localize_script()` which depends on an external JavaScript file loading. The inline JavaScript in the archive template executed before these variables were available.
