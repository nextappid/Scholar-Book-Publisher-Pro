...
class SBPP_Metadata_Generator {
    ...
    private function generate_book_metadata() {
        ...
        // Get all metadata
        $subtitle = get_post_meta($post->ID ?? 0, '_sbpp_book_subtitle', true);
        $description = get_post_meta($post->ID ?? 0, '_sbpp_book_description', true);
        $authors = get_post_meta($post->ID ?? 0, '_sbpp_authors', true);
        $editors = get_post_meta($post->ID ?? 0, '_sbpp_editors', true);
        ...
        $publisher_city = get_post_meta($post->ID ?? 0, '_sbpp_publisher_city', true);
        $pub_date = get_post_meta($post->ID ?? 0, '_sbpp_publication_date', true);
        $isbn = get_post_meta($post->ID ?? 0, '_sbpp_isbn', true);
        $doi = get_post_meta($post->ID ?? 0, '_sbpp_doi', true);
        $language = get_post_meta($post->ID ?? 0, '_sbpp_book_language', true);
        
        // Standardize language to ISO 639-1
        if (!empty($language)) {
            $lang_lower = strtolower(trim($language));
            $lang_map = array(
                'indonesian' => 'id', 'indonesia' => 'id', 'bahasa indonesia' => 'id',
                'english' => 'en', 'inggris' => 'en', 'bahasa inggris' => 'en',
                'malay' => 'ms', 'melayu' => 'ms', 'bahasa melayu' => 'ms',
                'arabic' => 'ar', 'arab' => 'ar',
                'spanish' => 'es', 'french' => 'fr', 'german' => 'de', 'dutch' => 'nl',
                'japanese' => 'ja', 'chinese' => 'zh', 'korean' => 'ko'
            );
            $language = isset($lang_map[$lang_lower]) ? $lang_map[$lang_lower] : $language;
        }

        $pages = get_post_meta($post->ID ?? 0, '_sbpp_page_count', true);
        $access_category = get_post_meta($post->ID ?? 0, '_sbpp_access_category', true);
        
        // Cover image
        $cover_id = get_post_meta($post->ID ?? 0, '_sbpp_book_cover', true);
        ...
        $pdf_url = '';
        $pdf_available = get_post_meta($post->ID ?? 0, '_sbpp_pdf_available', true);
        if ($pdf_available && $access_category === 'open') {
            $pdf_url = home_url('/sbpp-pdf/' . $post->ID . '/' . $post->post_name . '.pdf');
        }
        
        $year = !empty($pub_date) ? date('Y', strtotime($pub_date)) : '';
        $title = get_the_title($post->ID ?? 0);
        $subtitle = trim($subtitle);
        $full_title = !empty($subtitle) ? $title . ': ' . $subtitle : $title;
        ...
        if (!empty($full_title)) echo '<meta name="citation_title" content="' . esc_attr($full_title) . '">' . "\n";
        
        if (!empty($title)) echo '<meta name="citation_book_title" content="' . esc_attr($title) . '">' . "\n";
        
        if (!empty($isbn)) echo '<meta name="citation_isbn" content="' . esc_attr($isbn) . '">' . "\n";
        if (!empty($year)) echo '<meta name="citation_publication_date" content="' . esc_attr($year) . '">' . "\n";
        ...
        echo '<meta name="citation_type" content="book">' . "\n";
        echo '<meta name="citation_publication_type" content="book">' . "\n";
        
        // Authors (required by Google Scholar)
        if (!empty($authors) && is_array($authors)) {
            foreach ($authors as $author) {
                if (!empty($author['last_name']) && !empty($author['first_name'])) {
                    echo '<meta name="citation_author" content="' . esc_attr($author['first_name'] . ' ' . $author['last_name']) . '">' . "\n";
                }
            }
        }
        
        // Mendeley City & Subtitle
        if (!empty($publisher_city)) {
            echo '<meta name="citation_city" content="' . esc_attr($publisher_city) . '">' . "\n";
        }
        
        // URLs (critical for indexing)
        $permalink = get_permalink();
        if (!empty($permalink)) {
            echo '<meta name="citation_abstract_html_url" content="' . esc_url($permalink) . '">' . "\n";
            if ($access_category === 'open') {
                echo '<meta name="citation_fulltext_html_url" content="' . esc_url($permalink) . '">' . "\n";
            }
        }
        if ($access_category === 'open' && !empty($pdf_url)) {
        ...
        if (!empty($publisher_city)) echo '<meta name="DC.publisher.address" content="' . esc_attr($publisher_city) . '">' . "\n";
        ...
        if ($abstract) echo '<meta name="DC.description" content="' . esc_attr($abstract) . '">' . "\n";
        ...
        // ============================================
        // 8. SECURE COinS METADATA
        // ============================================
        echo "\n<!-- Secure COinS Metadata for Mendeley/Zotero -->\n";
        $coins_data = array(
            'ctx_ver'     => 'Z39.88-2004',
            'rft_val_fmt' => 'info:ofi/fmt:kev:mtx:book',
            'rft.genre'   => 'book',
            'rft.btitle'  => $full_title,
            'rft.pub'     => $publisher,
            'rft.date'    => $year,
            'rft.isbn'    => $isbn,
        );
        if ( ! empty( $publisher_city ) ) {
            $coins_data['rft.place'] = $publisher_city;
        }

        // Build query aman dengan separator &amp; dan RFC3986 untuk %20
        $coins_query = http_build_query( $coins_data, '', '&amp;', PHP_QUERY_RFC3986 );

        // Tambahkan authors secara manual karena key-nya sama (rft.au)
        if ( ! empty( $authors ) && is_array( $authors ) ) {
            foreach ( $authors as $author ) {
                if (!empty($author['first_name']) || !empty($author['last_name'])) {
                    $author_name = trim($author['first_name'] . ' ' . $author['last_name']);
                    $coins_query .= '&amp;rft.au=' . rawurlencode( $author_name );
                }
            }
        }

        // Render ke layar (tersembunyi, dilindungi esc_attr untuk cegah XSS)
        echo '<span class="Z3988" title="' . esc_attr( $coins_query ) . '"></span>' . "\n";
    ...
    private function generate_chapter_metadata() {
        ...
        if (!empty($chapter_title)) echo '<meta name="citation_title" content="' . esc_attr($chapter_title) . '">' . "\n";
        if (!empty($book->post_title)) echo '<meta name="citation_book_title" content="' . esc_attr($book->post_title) . '">' . "\n";
        if (!empty($chapter_number)) echo '<meta name="citation_chapter_number" content="' . esc_attr($chapter_number) . '">' . "\n";
        ...
        if (!empty($isbn)) echo '<meta name="citation_isbn" content="' . esc_attr($isbn) . '">' . "\n";
        $permalink = get_permalink();
        $access_category = get_post_meta($parent_book_id, '_sbpp_access_category', true);
        if (!empty($permalink)) {
            echo '<meta name="citation_abstract_html_url" content="' . esc_url($permalink) . '">' . "\n";
            if ($access_category === 'open') {
                echo '<meta name="citation_fulltext_html_url" content="' . esc_url($permalink) . '">' . "\n";
