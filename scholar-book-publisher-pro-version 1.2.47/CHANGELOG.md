...
## [1.2.44] - 2026-08-26
### Fixed
- Safely required orphaned class-sbpp-pdf-proxy.php in core with file_exists safeguard.

## [1.2.27] - 2026-05-17
### Fixed & Optimized
- **Strict Single Page Loading:** Enforced Google Scholar meta tags to only load on single book and chapter pages, completely preventing them from appearing on archive/catalog pages.
- **Date & Publisher Formatting:** Adjusted `citation_publication_date` to strictly output the year of publication and set default publisher to "Southeast Asian Publishing" for consistent indexing.
- **Strict Abstract Cleaning:** Enforced null-safe extraction from the Book Description and completely stripped all HTML tags (`wp_strip_all_tags`) for the `citation_abstract` meta tag.
- **Permanent Null Checks:** Implemented comprehensive null-checks (`?? 0`, `!empty()`) across all metadata generation variables to permanently prevent Critical Errors and Fatal Errors during rendering.
