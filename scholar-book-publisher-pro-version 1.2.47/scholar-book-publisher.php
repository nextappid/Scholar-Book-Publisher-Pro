 ...
 * Version: 1.2.47
 * Author: Southeast Asian Publishing
 * Author URI: https://halaman1.id
 * License: GPL v2 or later
 * License URI: http://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: scholar-book-publisher-pro
...
final class Scholar_Book_Publisher {
    ...
    public function includes() {
        ...
        // Load PDF Proxy Interceptor dengan perlindungan Fatal Error
        $sbpp_proxy_file = plugin_dir_path( __FILE__ ) . 'includes/class-sbpp-pdf-proxy.php';
        if ( file_exists( $sbpp_proxy_file ) ) {
            require_once $sbpp_proxy_file;
        }
        
        add_action( 'template_redirect', 'sbpp_direct_uri_pdf_interceptor' );
    }
...
/**
 * Modul SEO: Dynamic Academic Sitemap
 * Target URL: /sitemap-scholar.xml
 * @since 1.2.47
 */
function sbpp_register_sitemap_rule() {
    add_rewrite_rule('^sitemap-scholar\.xml$', 'index.php?sbpp_sitemap=1', 'top');
}
add_action('init', 'sbpp_register_sitemap_rule');

function sbpp_register_query_var($vars) {
    $vars[] = 'sbpp_sitemap';
    return $vars;
}
add_filter('query_vars', 'sbpp_register_query_var');

function sbpp_render_sitemap() {
    if (get_query_var('sbpp_sitemap') == 1) {
        if (ob_get_length()) {
            ob_clean();
        }
...
add_action('template_redirect', 'sbpp_render_sitemap');

/**
 * Modul SEO: Asynchronous Auto-Ping Search Engines
 * Trigger: transition_post_status (draft -> publish)
 * @since 1.2.47
 */
function sbpp_auto_ping_sitemap($new_status, $old_status, $post) {
    if ('scholar_book' !== $post->post_type) {
        return;
    }

    if ('publish' === $new_status && 'publish' !== $old_status) {
        $sitemap_url = home_url('/sitemap-scholar.xml');
...
add_action('transition_post_status', 'sbpp_auto_ping_sitemap', 10, 3);
