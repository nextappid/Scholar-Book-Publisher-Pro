...
= 1.2.47 =
* New: Modul Dynamic Academic Sitemap (khusus CPT scholar_book) terisolasi di /sitemap-scholar.xml.
* New: Modul Asynchronous Auto-Ping ke mesin pencari (Bing/Google) aktif saat transisi status buku ke publish.

= 1.2.46 =
* Major Feature: Deployed Enterprise cURL Write Callback streaming via Google Drive API to completely bypass login walls, preserve server RAM, prevent JSON header leaks, and ensure native PDF indexing by Google Scholar.

= 1.2.45 =
* Major Fix: Resolved fatal error by amputating deprecated duplicate function body in core file while safeguarding the template_redirect hook.

= 1.2.44 =
* Fix: Safely required orphaned class-sbpp-pdf-proxy.php in core with file_exists safeguard.

= 1.2.43 =
* Major Fix: (Placeholder for previous fixes if any)

= 1.2.42 =
* Major Fix: Implemented dual-prefix meta key scanning and auto-construction of Google Drive direct download links for flawless Google Scholar crawler compatibility.

= 1.2.41 =
* Major Fix: Implemented bulletproof dual-schema meta key fallback (WP Upload & Google Drive) and output buffer cleaner for PDF URI Interceptor in proxy class.

= 1.2.40 - 2026-08-17 =
* Fix: Perfected PDF URI Interceptor via anti-cached 301 redirect based on exact scholar_book CPT and accurate meta key extraction.

= 1.2.39 - 2026-08-12 =
* Major Fix: Implemented init-level URI Interceptor with Book ID and query-string immunity to solve Canonical Hijack on PDF endpoints.

= 1.2.38 - 2026-08-11 =
* Fix: Reverse engineered PDF Endpoint to use 301 Redirect to direct URL using _sbpp_pdf_wordpress_id meta key for robust Google Scholar indexing
* Fix: Enforced RFC3986 encoding for COinS metadata to ensure spaces become %20 instead of +

= 1.2.37 - 2026-08-11 =
* Fix: Enterprise-grade PDF path resolution via attachment_url_to_postid and injected secure COinS metadata for strict Mendeley Book identification

= 1.2.36 - 2026-08-11 =
* Cache bypass PDF endpoint, HTTP HEAD optimization, exact matching [BOOK] logic.

= 1.2.35 - 2026-08-06 =
* Optimized Google Scholar SEO: Clean pseudo-static PDF routing for Open Access, safe abstract-only indexing for Close Access, and ISO 639-1 language standards.

= 1.2.34 - 2026-08-05 =
* Fix: Chicago citation format fallback for unknown city
* Fix: Added CY (City) tag support for RIS exports
