...
// Load citation data
$citation_data = class_exists('SBPP_Citation_Generator') ? SBPP_Citation_Generator::get_citations(get_the_ID()) : ['html' => [], 'raw' => []];
$citation_json = wp_json_encode($citation_data);
?>
...
<style>
    ...
    * {
    ...
    .scholar-modal-overlay.active .scholar-modal-content {
        transform: scale(1);
    }
    ...
    .scholar-format-tabs {
        ...
        gap: 0.5rem;
        margin-bottom: 1.5rem;
        border-bottom: 2px solid var(--scholar-border-light);
    }
    ...
    .scholar-format-tab:hover {
        color: var(--scholar-primary);
    }

    .scholar-format-tab.active {
        color: var(--scholar-accent);
        border-bottom-color: var(--scholar-accent);
    }

    .scholar-citation-result {
        background: var(--scholar-citation-bg);
        border: 1px solid var(--scholar-border);
        border-left: 4px solid var(--scholar-accent);
        padding: 1.5rem;
        border-radius: 4px;
        font-family: 'Times New Roman', Times, serif; /* Typical for citations */
        font-size: 1rem;
        line-height: 1.6;
        color: var(--scholar-primary);
        margin-bottom: 1.5rem;
    ...
    <div class="scholar-book-grid">
        ...
        <aside class="scholar-book-sidebar">
            ...
            <div class="scholar-actions">
                <?php if ($pdf_url): 
                    $download_url = class_exists('SBPP_Usage_Metrics') ? SBPP_Usage_Metrics::get_tracked_download_url(get_the_ID()) : $pdf_url;
                ?>
                ...
                <button onclick="openCitationModal()" class="scholar-btn scholar-btn-secondary">
                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <div class="scholar-modal-overlay" id="scholarCitationModal">
        <div class="scholar-modal-content">
            <div class="scholar-modal-header">
                <h3 class="scholar-modal-title">Cite this Book</h3>
                <button class="scholar-modal-close" onclick="closeCitationModal()" aria-label="Close modal">
            ...
            <div class="scholar-format-tabs">
                <button class="scholar-format-tab active" onclick="switchCitationFormat('apa')">APA</button>
                <button class="scholar-format-tab" onclick="switchCitationFormat('mla')">MLA</button>
                <button class="scholar-format-tab" onclick="switchCitationFormat('chicago')">Chicago</button>
            </div>
            
            <div class="scholar-citation-result" id="scholarCitationResult">
                <!-- Citation HTML injected here via JS -->
            </div>
            
            <div class="scholar-citation-actions">
                <button id="scholarBtnCopyCite" class="scholar-btn scholar-btn-primary" onclick="copyCitation()">
                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M4 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2zm2-1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1zM2 5a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-1h1v1a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h1v1z"/>
                    </svg>
                    Copy Citation
                </button>
                <button class="scholar-btn scholar-btn-secondary" onclick="downloadCitationFile('ris')">Download RIS</button>
                <button class="scholar-btn scholar-btn-secondary" onclick="downloadCitationFile('bib')">Download BibTeX</button>
            </div>
        </div>
    </div>

    <script>
    // Citation Data embedded from PHP
    const citationData = <?php echo $citation_json; ?>;
    let currentFormat = 'apa';

    function openCitationModal() {
        const modal = document.getElementById('scholarCitationModal');
    ...
    function closeCitationModal() {
        ...
        // Reset copy button
        const copyBtn = document.getElementById('scholarBtnCopyCite');
    ...
    function switchCitationFormat(format) {
        ...
        // Update tabs UI
        document.querySelectorAll('.scholar-format-tab').forEach(tab => {
            tab.classList.remove('active');
            if (tab.innerText.toLowerCase() === format) {
    ...
    function renderCitation() {
        const resultDiv = document.getElementById('scholarCitationResult');
        if (citationData.html && citationData.html[currentFormat]) {
            resultDiv.innerHTML = citationData.html[currentFormat];
        } else {
            resultDiv.innerHTML = '<em>Citation not available.</em>';
        }
    }

    function copyCitation() {
        const resultDiv = document.getElementById('scholarCitationResult');
        // Using innerText to get clean text without HTML tags for clipboard
        const plainText = resultDiv.innerText || resultDiv.textContent;
        
        navigator.clipboard.writeText(plainText).then(() => {
            const btn = document.getElementById('scholarBtnCopyCite');
            btn.innerHTML = '<svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z"/></svg> Copied!';
            setTimeout(() => {
    ...
    function downloadCitationFile(type) {
        ...
        let authorsList = data.authors.map(a => `${a.last}, ${a.first}`).join(' and ');
        
        if (type === 'ris') {
            content += 'TY  - ' + data.type + '\n';
            content += 'TI  - ' + data.title + '\n';
            data.authors.forEach(a => {
                content += 'AU  - ' + a.last + ', ' + a.first + '\n';
            });
            if (data.year) content += 'PY  - ' + data.year + '\n';
            if (data.city) content += 'CY  - ' + data.city + '\n';
            if (data.publisher) content += 'PB  - ' + data.publisher + '\n';
            if (data.url) content += 'UR  - ' + data.url + '\n';
            if (data.type === 'CHAP') {
                if (data.book_title) content += 'T2  - ' + data.book_title + '\n';
                if (data.pages) {
                    const pg = data.pages.split('-');
                    if (pg[0]) content += 'SP  - ' + pg[0] + '\n';
                    if (pg[1]) content += 'EP  - ' + pg[1] + '\n';
                }
            }
            content += 'ER  - \n';
        } else if (type === 'bib') {
            const id = data.authors[0] ? data.authors[0].last.toLowerCase() + data.year : 'citation' + data.year;
            const bibType = data.type === 'CHAP' ? 'inbook' : 'book';
            
            content += `@${bibType}{${id},\n`;
            content += `  title={${data.title}},\n`;
            content += `  author={${authorsList}},\n`;
            if (data.year) content += `  year={${data.year}},\n`;
            if (data.publisher) content += `  publisher={${data.publisher}},\n`;
            if (data.type === 'CHAP' && data.book_title) content += `  booktitle={${data.book_title}},\n`;
            if (data.pages) content += `  pages={${data.pages}},\n`;
            content += `}\n`;
        }
        ...
        a.href = url;
        a.download = `citation-${data.year || 'export'}.${type}`;
    ...
    document.getElementById('scholarCitationModal').addEventListener('click', function(e) {
        if (e.target === this) {
            closeCitationModal();
        }
    });
