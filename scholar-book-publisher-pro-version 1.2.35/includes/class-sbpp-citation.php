...
class SBPP_Citation_Generator {
    ...
    public static function get_citations($post_id) {
        ...
        if ($is_chapter) {
            ...
            $city_str = $city ? "{$city}: " : "";
            $citations['chicago'] = "{$chi_authors}. \"{$chapter_title}.\" Dalam <i>{$book_title}</i>{$editor_chi}{$pages_chi}. {$city_str}{$publisher}, {$year}.";
        } else {
            // Book Format
            ...
            $city_str = $city ? "{$city}: " : "";
            $citations['chicago'] = "{$chi_authors}. <i>{$book_title}</i>. {$city_str}{$publisher}, {$year}.";
        }
