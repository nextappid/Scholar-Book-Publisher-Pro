...
<?php while (have_posts()) : the_post(); 
    // Get all metadata
    $cover_id = get_post_meta(get_the_ID(), '_sbp_book_cover', true);
    $subtitle = get_post_meta(get_the_ID(), '_sbp_book_subtitle', true);
    $description = get_post_meta(get_the_ID(), '_sbp_book_description', true);
    $language = get_post_meta(get_the_ID(), '_sbp_book_language', true);
    $authors = get_post_meta(get_the_ID(), '_sbp_authors', true);
    $editors = get_post_meta(get_the_ID(), '_sbp_editors', true);
    $publisher = get_post_meta(get_the_ID(), '_sbp_book_publisher', true);
    $publisher_city = get_post_meta(get_the_ID(), '_sbp_publisher_city', true);
    $pub_date = get_post_meta(get_the_ID(), '_sbp_publication_date', true);
    $isbn = get_post_meta(get_the_ID(), '_sbp_isbn', true);
    $doi = get_post_meta(get_the_ID(), '_sbp_doi', true);
    $dimensions = get_post_meta(get_the_ID(), '_sbp_dimensions', true);
    $price = get_post_meta(get_the_ID(), '_sbp_price', true);
    $access_category = get_post_meta(get_the_ID(), '_sbp_access_category', true);
    
    // PDF data
    $pdf_available = get_post_meta(get_the_ID(), '_sbp_pdf_available', true);
    $pdf_source = get_post_meta(get_the_ID(), '_sbp_pdf_source', true);
    $pdf_url = '';
    
    if ($pdf_available) {
        if ($pdf_source === 'wordpress') {
            $pdf_wordpress_id = get_post_meta(get_the_ID(), '_sbp_pdf_wordpress_id', true);
            if ($pdf_wordpress_id) {
                $pdf_url = wp_get_attachment_url($pdf_wordpress_id);
            }
        } elseif ($pdf_source === 'gdrive') {
            $pdf_gdrive_id = get_post_meta(get_the_ID(), '_sbp_pdf_gdrive_id', true);
            if ($pdf_gdrive_id) {
...
<div class="scholar-book-container" data-theme="dark">
    ...
    <div class="scholar-book-grid">
        ...
        <aside class="scholar-book-sidebar">
            ...
            <!-- Usage Metrics -->
            <?php
            $metrics = SBP_Usage_Metrics::get_metrics(get_the_ID());
            ...
            <div class="scholar-actions">
                <?php if ($pdf_url): ?>
                    <a href="<?php echo esc_url(SBP_Usage_Metrics::get_tracked_download_url(get_the_ID())); ?>" 
        ...
        <main class="scholar-book-main-column">
            ...
            $chapters = new WP_Query(array(
                ...
                'meta_key' => '_sbp_chapter_first_page',
                'order' => 'ASC'
            ));
                ...
                <div class="scholar-chapters">
                    ...
                    <ol class="scholar-chapters-list">
                        ...
                        while ($chapters->have_posts()): $chapters->the_post(); 
                            $first_page = get_post_meta(get_the_ID(), '_sbp_chapter_first_page', true);
                            $last_page = get_post_meta(get_the_ID(), '_sbp_chapter_last_page', true);
                            $chapter_authors = get_post_meta(get_the_ID(), '_sbp_chapter_authors', true);
                        ?>
