...
function sbpp_direct_uri_pdf_interceptor() {
    ...
    if ( preg_match( '#/sbpp-pdf/(\d+)/([^/]+)\.pdf$#i', $parsed_url, $matches ) ) {
        ...
        $pdf_data = get_post_meta( $post_id, '_sbpp_pdf_wordpress_id', true );
        $pdf_url = '';

        if ( ! empty( $pdf_data ) ) {
            if ( is_numeric( $pdf_data ) ) {
                // Jika berupa Attachment ID Media Library
                $pdf_url = wp_get_attachment_url( intval( $pdf_data ) );
            } elseif ( filter_var( $pdf_data, FILTER_VALIDATE_URL ) ) {
                // Jika berupa Direct URL (misal Google Drive)
                $pdf_url = $pdf_data;
            } elseif ( strpos( $pdf_data, '/wp-content/' ) !== false ) {
                // Fallback: Jika data berupa relative path
                $pdf_url = site_url( $pdf_data );
            }
        }

        // Eksekusi 301 Redirect Anti-Ghost File
        if ( ! empty( $pdf_url ) ) {
