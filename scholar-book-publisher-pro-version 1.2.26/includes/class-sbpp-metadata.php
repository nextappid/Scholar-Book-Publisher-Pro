    ...
    private function generate_book_metadata() {
        ...
        if (!empty($authors) && is_array($authors)) {
            foreach ($authors as $author) {
                if (is_array($author) && !empty($author['last_name']) && !empty($author['first_name'])) {
        ...
        if (!empty($editors) && is_array($editors)) {
            foreach ($editors as $editor) {
                if (is_array($editor) && !empty($editor['first_name']) && !empty($editor['last_name'])) {
        ...
        if ($access_category === 'open' && $pdf_url) {
        ...
        if (!empty($authors) && is_array($authors)) {
            foreach ($authors as $author) {
                if (is_array($author) && !empty($author['first_name']) && !empty($author['last_name'])) {
        ...
        $first_author_name = '';
        if (!empty($authors) && is_array($authors)) {
            $first_author_data = reset($authors);
            if (is_array($first_author_data) && !empty($first_author_data['first_name']) && !empty($first_author_data['last_name'])) {
                $first_author_name = $first_author_data['first_name'] . ' ' . $first_author_data['last_name'];
            }
        }

        // AI-specific hints
        echo '<meta name="article:published_time" content="' . esc_attr($pub_date) . '">' . "\n";
        echo '<meta name="article:author" content="' . esc_attr($first_author_name) . '">' . "\n";
        
        // General description for AI understanding
        $meta_description = $abstract ? wp_strip_all_tags(wp_trim_words($abstract, 40)) : 
                           'Scholarly book: ' . $full_title . ' by ' . ($first_author_name ? $first_author_name : 'various authors');
