...
new SBPP_Sitemap_Generator(); // ← Instantiated in file!
```
...
new SBPP_Sitemap_Generator(); // ← Instantiated in main plugin
```
...
↓
Rewrite to: index.php?sbpp_sitemap=books
↓
get_query_var('sbpp_sitemap') === 'books'
↓
...
// Should see 'sbpp_sitemap' in the list
```
...
add_rewrite_rule(
    ...
    'index.php?sbpp_sitemap=books',  // Rewrite to
    'top'                           // Priority
);
```
...
add_action('template_redirect', function() {
    if (get_query_var('sbpp_sitemap') === 'books') {
        // Set headers
