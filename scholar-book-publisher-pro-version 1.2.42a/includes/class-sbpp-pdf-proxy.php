...
function sbpp_direct_uri_pdf_interceptor() {
    ...
    if ( preg_match( '#/sbpp-pdf/(\d+)/([^/]+)\.pdf$#i', $parsed_url, $matches ) ) {
        ...
        // --- HACKER KILL SWITCH ---
        if ( isset($_GET['sbpp_debug']) && $_GET['sbpp_debug'] === '123' ) {
            echo "<div style='background:#111; color:#0f0; padding:20px; font-family:monospace; font-size:16px;'>";
            echo "<h1>SYSTEM DEBUG OVERRIDE</h1>";
            echo "<strong>Post ID:</strong> " . $post_id . "<br>";
            echo "<strong>Post Type:</strong> " . $post->post_type . "<br>";
            echo "<strong>RAW Access 1:</strong> "; var_dump(get_post_meta($post_id, '_sbpp_access_category', true)); echo "<br>";
            echo "<strong>RAW Access 2:</strong> "; var_dump(get_post_meta($post_id, 'sbpp_access_category', true)); echo "<br>";
            echo "<strong>RAW G-Drive ID 1:</strong> "; var_dump(get_post_meta($post_id, 'sbpp_pdf_gdrive_id', true)); echo "<br>";
            echo "<strong>RAW G-Drive ID 2:</strong> "; var_dump(get_post_meta($post_id, '_sbpp_pdf_gdrive_id', true)); echo "<br>";
            echo "</div>";
            die(); 
        }
        // ---------------------------
